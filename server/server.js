...
app.post("/api/lenders", async (req, res) => {
  try {
    ...
    if (!lenderUser || !lenderUser.location?.coordinates?.length) {
      return res
        .status(400)
        .json({ message: "Your location is not set properly" });
    }
...
app.get("/api/lenders/responses", async (req, res) => {
  try {
    ...
    if (!lenderOffers || lenderOffers.length === 0) {
      return res
        .status(404)
        .json({ message: "No responses found for this lender" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching lender responses" });
  }
});
